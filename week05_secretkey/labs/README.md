...
```
sudo ip addr add 10.10.1.7/24 dev eth11
sudo route add default gw 10.10.1.254 eth11
...
| DMZ	| There should be an FTP server working on Ubuntu and Windows 2003. From WINDOWS, access the FTP server on the UBUNTU server: "telnet 10.10.x.7 21" then enter: "USER napier" then "PASS napier123" "QUIT" | Outline the messages that you received: What happens to each of these when you try with an incorrect username and password: |
| LAN | From UBUNTU access the WINDOWS host with "telnet 10.10.x.7 21" then enter: "USER Administrator" and "PASS napier" and QUIT	| Outline the messages that you received: What happens to each of these when you try with an incorrect username and password: |
