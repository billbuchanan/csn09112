...
| DMZ	|  There should be an FTP server working on Ubuntu and Windows 2003. From WINDOWS, access the FTP server on the UBUNTU server: "telnet 10.10.x.7 21"  then enter: "USER napier" then "PASS napier123" and then "QUIT" | 	Outline the messages that you received: What happens to each of these when you try with an incorrect username and password:  | 
| LAN | 	From UBUNTU access the WINDOWS host with "telnet 10.10.x.7 21" then enter: "USER Administrator" then "PASS napier" and then "QUIT" | 	Outline the messages that you received: What happens to each of these when you try with an incorrect username and password: | 
